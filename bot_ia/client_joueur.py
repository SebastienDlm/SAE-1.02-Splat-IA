...
def mon_IA(ma_couleur, carac_jeu, le_plateau, les_joueurs):
    ...
    """
    # var global
    pos = joueur.get_pos(les_joueurs[ma_couleur])
    reserve = joueur.get_reserve(les_joueurs[ma_couleur])
    surface = joueur.get_surface(les_joueurs[ma_couleur])
    position = joueur.get_pos(les_joueurs[ma_couleur])
    distance_objet = 5
    distance_scan = 20
    ...
    # tir
    le_tir = cerveau.tir(le_plateau, pos, direction_tir, reserve, ma_couleur, les_joueurs)
    
    # déplacement
    deplacement_choisi = cerveau.deplacement(le_plateau, pos, les_joueurs, reserve, surface, position, ma_couleur, distance_objet, distance_scan)

    return le_tir + deplacement_choisi
